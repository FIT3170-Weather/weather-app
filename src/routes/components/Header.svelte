<script lang="ts">
	...
	const handleInput = () => {
		...
		if (searchString.length > 0) {
			userClosed = false;
			findItemItems = search_data.filter((item: string) =>
	...
	<div class="drawer-content fixed navbar bg-base-200 flex-row max-h-[48px] z-50 justify-between">
		<div class="pr-10 justify-self-start">
			...
			<a class="btn btn-ghost text-xl px-1" href="/"
				><img src="/images/climate_text_logo.png" alt="logo" width="100" /></a
			>
		</div>
		...
		<div class="self-start relative form-control flex-grow pl-16 hidden md:flex">
			...
			{#if !userClosed}
				...
				<button
					...
					class="absolute pt-12 self-end pr-4 transform -translate-y-1/2 text-gray-500">✕</button
				>
				{#if filteredItems.length > 0}
					<div class="bg-white flex flex-col rounded overflow-hidden z-50 w-full pl-3 pr-10">
						{#each filteredItems as items}
							<button
								on:click={() => changeLocationNormalView(items)()}
								class="block z-20 cursor-pointer text-black my-2 text-left">{items}</button
							>
						{/each}
					</div>
				{:else}
		...
		<ul class="menu bg-base-200 min-h-full w-6/7">
			...
			<li>
				<div class="flex flex-row justify-between">
					<a class="btn btn-ghost pl-0" href="/"
						><img src="/images/climate_text_logo.png" alt="logo" width="100" /></a
					>
					<!-- Date -->
					<div class="flex flex-col">
			...
			<li>
				<a href="/" class="w-fufull h-12 rounded-2xl items-center">
					<div class="flex flex-row items-center space-x-5">
						<img src="/images/dashboard.png" alt="Dashboard" class="icon" width="30" />
			...
			<li>
				<a href="/forecast" class="w-full h-12 rounded-2xl items-center">
					<div class="flex flex-row items-center space-x-5">
						<img src="/images/forecast.png" alt="Forecast" class="icon" width="30" />
			...
			<li>
				<a href="/history" class="w-full h-12 rounded-2xl items-center">
					<div class="flex flex-row items-center space-x-5">
						<img src="/images/history.png" alt="History" class="icon" width="30" />
			...
			{#if $user}
				<li>
					<a href="/profile" class="w-full h-12 rounded-2xl items-center">
						<div class="flex flex-row items-center space-x-5">
							<img src="/images/profile_icon.png" alt="Profile" class="icon" width="30" />
				...
				<li>
					<a href="/alerts" class="w-full h-12 rounded-2xl items-center">
						<div class="flex flex-row items-center space-x-5">
							<img src="/images/alerts_icon.png" alt="Alerts" class="icon" width="30" />
			...
			<li>
				<button class="w-full h-12 rounded-2xl items-center" on:click={openLoginModal}>
					<div class="flex flex-row items-center space-x-5">
						<img src="/images/log-out.png" alt="Logout" class="icon" width="30" />
						{#if $user}
	...
	<div class="modal-box">
		...
		<div class="modal-action">
			<form method="dialog" on:submit={handleLocationChangeSubmit}>
				<button class="custom-btn btn" disabled={!filteredItems.includes(searchString)}
					>SEARCH</button
				>
			</form>
		</div>
	...
	<div class="modal-box bg-base-100">
		...
		{:else}
			...
			<div class="text-center">
				<button
					class="w-[122px] h-[40px] m-[5px]"
					on:click={() => authHandlers.loginWithGoogle(closeLoginModal)}
				>
					<img src="/images/google.png" alt="google" />
				</button>
			</div>
			<p class="text-xs font-extralight">
				By signing in you agree to CliMate's terms of service and privacy policy
			</p>
		{/if}
	</div>
</dialog>
