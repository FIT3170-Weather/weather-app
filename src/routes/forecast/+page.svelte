<script lang="ts">
	...
	const url = `http://127.0.0.1:8000/weather-forecast?location_code=${data.location}`;
	let selected = 'daily';
...
<section class="flex flex-col items-center w-full">
	...
	<div class="grid md:grid-cols-12 grid-cols-1 justify-between items-center w-full px-5">
		<p class="col-span-2">Last update: {formatDateTime(lastUpdate)}</p>
		<!-- Display last update time -->
		<!-- spacer -->
		<div class="col-span-8"></div>
		<div class="col-span-2 md:text-right text-left">
			<button
				class="py-2 md:text-base text-sm btn text-error-content max-w-40 max-h-2"
				on:click={manualUpdate}>Update Now</button
			>
						...
						<div class="md:col-span-2 col-span-1 text-black">
							<ForcastInfoHourly
								time={forecastData.hourly.time
									? formatTime(forecastData.hourly.time[index])
									: 'N/A'}
								temp={`${forecastData.hourly.temperature ? forecastData.hourly.temperature[index] + '°C' : 'N/A'}`}
								condition={createHourlyDescription(
									forecastData.hourly.wind_speed[index],
									forecastData.hourly.humidity[index]
								)}
								real_feel_temp={`${forecastData.hourly.real_feel ? forecastData.hourly.real_feel[index] + '°C' : 'N/A'}`}
								weather_condition={determineHourlyWeatherCondition(
									forecastData.hourly.precipitation[index]
								)}
							/>
						</div>
						<div class="md:col-span-3 col-span-1">
							<ForecastStat
								humidity_percentage={forecastData.hourly.humidity
									? forecastData.hourly.humidity[index]
									: 'N/A'}
								wind_speed={forecastData.hourly.wind_speed
									? forecastData.hourly.wind_speed[index]
									: 'N/A'}
								wind_direction={forecastData.hourly.wind_direction
									? forecastData.hourly.wind_direction[index]
									: 'N/A'}
								pressure={forecastData.hourly.pressure
									? forecastData.hourly.pressure[index]
									: 'N/A'}
								percipitation={forecastData.hourly.precipitation
									? forecastData.hourly.precipitation[index]
									: 'N/A'}
							/>
						</div>
					...
					<div class="grid grid-cols-1 md:grid-cols-5 gap-4">
						<div class="md:col-span-2 col-span-1">
							<ForcastInfoDaily
								...
								description={createDailyDescription(
									forecastData.daily.precipitation[index],
									forecastData.daily.wind_speed[index]
								)}
								weather_condition={determineDailyWeatherCondition(
									forecastData.daily.precipitation[index]
								)}
							/>
						</div>
						<div class="md:col-span-3 col-span-1">
							<ForecastStat
								humidity_percentage={forecastData.daily.humidity
									? forecastData.daily.humidity[index]
									: 'N/A'}
								wind_speed={forecastData.daily.wind_speed
									? forecastData.daily.wind_speed[index]
									: 'N/A'}
								wind_direction={forecastData.daily.wind_direction
									? forecastData.daily.wind_direction[index]
									: 'N/A'}
								pressure={forecastData.daily.pressure ? forecastData.daily.pressure[index] : 'N/A'}
								percipitation={forecastData.daily.precipitation
									? forecastData.daily.precipitation[index]
									: 'N/A'}
							/>
						</div>
